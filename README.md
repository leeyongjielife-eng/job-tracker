...
- 默认扫描最近 `60` 天发布的岗位
- 每周运行一次，但不是“只保留最近 7 天发布的岗位”
- 因为同步到 Notion / 后续数据库时按 `Link` 去重且不会删除旧记录，所以每周运行会在原有岗位库基础上新增新岗位
...
- `60 天`：默认发布窗口，也是首次回填窗口
- `每周一次`：更新频率，不是发布时间过滤条件
- 后续周跑：继续扫描滚动 `60` 天，并把新岗位在原有岗位库基础上追加进去
